...
namespace SMSPDULib
{
	public class SMS : SMSBase
	{
        ...
        protected SMSEncoding _messageEncoding;
		#endregion
        ...
        /// updated by [ummarbhutta@gmail.com]
        /// </summary>
		public string Message
		{
			...
			set
			{
                ...
                {
                    throw new GSMEncodingException("Encoding must be set before setting body of SMS");
                }
                else if (_messageEncoding == SMSEncoding._7bit)
                {
                    //Convert the string to GSM 03.38 equivalent string
                    _message = GSM0338Charset.ConvertToGSM0338String(value);
                }
                else if (_messageEncoding == SMSEncoding.UCS2 || _messageEncoding == SMSEncoding._8bit)
                {
                    _message = value;
                }
			}
        ...
        /// <summary>
        /// Gets or Sets encoding of this message
        /// </summary>
        public SMSEncoding MessageEncoding
        {
            get
            {
		...
		public bool InParts
		{
            ...
            //Updated code block start ummarbhutta@gmail.com - October 18, 2009
            set
            {
                if (value == true)
                {
            ...
            //Updated code block end ummarbhutta@gmail.com - October 18, 2009
		}
        ...
        #region Flash SMS Settings
        /// <summary>
        /// Gets or Sets the Flash SMS bit from Data Coding Scheme Octet
        /// ummarbhutta@gmail.com
        /// </summary>
        public bool Flash
        {
            get
            {
                if (_dataCodingScheme != null)
                {
                    return ((_dataCodingScheme & 0x10) > 0);
                }
                else
                {
                    return false;
            ...
            set
            {
                //0x10 mask set SMS to Flash SMS
                _dataCodingScheme = (value) ? (byte)(_dataCodingScheme | 0x10) : (byte)(_dataCodingScheme | 0x00);
            }
        }
        #endregion
        ...
        /// 7-bit encoding implemented by [ummarbhutta@gmail.com]
        /// </summary>
        /// <returns>string of packed SMS</returns>
		public virtual string Compose()
		{
            if (_messageEncoding == SMSEncoding.UCS2)
            {
            ...
            if (_messageEncoding == SMSEncoding._7bit)
            {
            ...
            //New code block for preserving data coding scheme value like Flash SMS
            //[ummarbhutta@gmail.com]
            int mEncoding = (int)_messageEncoding | _dataCodingScheme;
            encodedData += Convert.ToString(mEncoding, 16).PadLeft(2, '0'); //Data coding scheme
            //New code block for preserving data coding scheme value like Flash SMS
            //[ummarbhutta@gmail.com]
        ...
        /// Support only 7-bit and UCS2 encoding
        /// [ummarbhutta@gmail.com]
        /// </summary>
        /// <returns>string[] of SMS PDUs</returns>
        public string[] ComposeLongSMS()
        {
            ...
            if (_messageEncoding == SMSEncoding._7bit)
            {
                parts = (int)(Math.Ceiling(((double)(_message.Length) / 153)));
            }
            else if (_messageEncoding == SMSEncoding.UCS2)
            {
                parts = (int)(Math.Ceiling(((double)(_message.Length) / 67)));
            }
            TotalParts = parts;
            ...
            for (int i = 1; i <= parts; i++)
            {
                ...
                if (_messageEncoding == SMSEncoding._7bit)
                {
                    header = EncodeMultiPartHeader(_udh);
                }
                else if (_messageEncoding == SMSEncoding.UCS2)
                {
                    header = Encoding.BigEndianUnicode.GetString(_udh);
                }

                int lengthToCut = 0;
                if (_messageEncoding == SMSEncoding._7bit)
                {
                    lengthToCut = ((((i - 1) * 153) + 153) > origionalMessage.Length) ? origionalMessage.Length - ((i - 1) * 153) : 153;
                    _message = origionalMessage.Substring((i - 1) * 153, lengthToCut);
                }
                else if (_messageEncoding == SMSEncoding.UCS2)
                {
                    lengthToCut = ((((i - 1) * 67) + 67) > origionalMessage.Length) ? origionalMessage.Length - ((i - 1) * 67) : 67;
                    _message = origionalMessage.Substring((i - 1) * 67, lengthToCut);
                }
